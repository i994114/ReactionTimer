...
import { formatTime } from "./Training";
    ...
    <ul className="cards">
      {timers.map((timer) => (
        <li className="card" key={timer.id}>
          ...
          <p className="card__time card__set--small">トレーニング：{formatTime(timer.trainingTime)}</p>
          <p className="card__time card__set--small">インターバル：{formatTime(timer.interval)}</p>
